...
- HISTORY RECONCILIATION: when the memo quotes a figure attributed to an EARLIER NOTE ("the last note said", "X then", "at the time"), the figure must either match a consistent-basis recomputation against <computed_figures> or be explicitly corrected in the same breath. Flag as CRITICAL a superseded or mis-based figure reprinted as history (e.g. quoting "1.8x then" when the consistent basis was 2.4x — implying the stock got cheaper when only the arithmetic changed).
