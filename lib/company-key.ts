...
export function identityFromProfile(profile: unknown): CompanyIdentity {
  ...
  const key =
    isin ??
    (name ? `name:${normalizeCompanyName(name)}` : `ticker:${(p?.symbol ?? "").toUpperCase()}`);
  return { key, isin, name };
}
...
export async function identityForTicker(ticker: string): Promise<CompanyIdentity> {
  try {
    ...
    return { key: `ticker:${ticker.toUpperCase()}`, isin: null, name: null };
  }
}
...
export async function crossListingSpread(
  ...
  try {
    const toUsd = async (ticker: string): Promise<{ usd: number; label: string } | null> => {
      const [quote, profile] = await Promise.all([
        fmpGet<{ price?: number }[]>("quote", { symbol: ticker }),
      ...
      return { usd: (price / penceFactor) * fx, label: `${currency} ${price}` };
    };
