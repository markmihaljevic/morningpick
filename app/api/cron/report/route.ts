...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const [{ data: deliveries }, { data: memos }, { data: budget }] = await Promise.all([
    db()
      .from("deliveries")
    ...
    db()
      .from("memos")
      .select("ticker, title, sent_at, subscribers(email)")
      .eq("delivery_date", today),
  ...
  const lines = [
    ...
    ...(memos ?? []).map((m) => {
      ...
      return `  • ${m.ticker} → ${(sub as { email?: string })?.email ?? "?"} ${m.sent_at ? "✓" : "(NOT SENT)"}`;
    }),
    ``,
    ...(failed.length > 0
  ...
  const problems = failed.length > 0 || stuck.length > 0 || (memos ?? []).length === 0;
